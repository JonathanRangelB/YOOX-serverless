...
module.exports.handler = async (event: APIGatewayEvent) => {
  ...
  const { spInsertNewLoanRequest } = JSON.parse(event.body) as { spInsertNewLoanRequest: SPInsertNewLoanRequest };

  if (!spInsertNewLoanRequest) {
    return generateJsonResponse({ message: 'spInsertNewLoanRequest is not defined' }, 400);
  }
  
  const validatedData = isValidLoanData(spInsertNewLoanRequest)

  if (!validatedData.valid) {
    return generateJsonResponse({ message: 'Object provided invalid', errors: validatedData.errors }, 400);
  }
