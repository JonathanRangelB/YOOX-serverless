import { Int, Date, Float, DateTime, Table, VarChar, TransactionError } from 'mssql';
...
export const registerNewLoanRequest = async (
    ...
    try {
        ...
        const nextIdQuery = await procTransaction.request().query<loan_request>(`SELECT * FROM LOAN_REQUEST WHERE ID = (SELECT MAX(ID) FROM LOAN_REQUEST)`);
        const nextId = nextIdQuery.recordset.length > 0 ? nextIdQuery.recordsets[0][0].ID + 1 : 1;
        ...
        tableNewRequestLoan.columns.add('FECHA_FINAL_ESTIMADA',Date, {nullable: false});
        ...
        tableNewRequestLoan.columns.add('CREATED_DATE',DateTime, {nullable: true});
        ...
        tableNewRequestLoan.columns.add('APPROVED_DATE',DateTime, {nullable: true});
