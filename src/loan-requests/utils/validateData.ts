...
function validateDataResult(
  ...
  if (fecha_inicial > fecha_final_estimada) {
    return { result: false, message: 'La fecha inicial no puede ser posterior a la fecha final estimada' }
  }

  if (!idAgente) {
    return { result: false, message: 'El agente no existe o se encuentra inactivo' }
  }

  if (!idUsuario) {
    return { result: false, message: 'El usuario no existe o se encuentra inactivo' }
  }
