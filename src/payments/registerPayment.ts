...
import { genericQuery, getPhoneNumberByPersonId } from "../helpers/asyncUtils";
...
export const registerPayment = async (
	...
	try {
		...
		const dataForWAMessage = await genericQuery<{
			NOMBRE: string;
			SEMANAS_RESTANTES: string;
		}>(
			`select c.NOMBRE, (SELECT COUNT(*) FROM PRESTAMOS_DETALLE pd WHERE pd.ID_PRESTAMO = ${spaAltaPago.ID_PRESTAMO}) AS SEMANAS_RESTANTES from CLIENTES c where c.ID = ${spaAltaPago.ID_CLIENTE}`,
		);

		message = `Hola ${dataForWAMessage.NOMBRE}\n\nHemos recibido su pago para el prestamo #${spaAltaPago.ID_PRESTAMO} correspondiente a la semana *${spaAltaPago.NUMERO_SEMANA}* de *${dataForWAMessage.SEMANAS_RESTANTES}*\n\nFinanciera YOOX agradece su pago.`;
