...
export const registerNewEndorsement = async (
    ...
    try {
        ...
        const addAddressResult = await registerNewAddress(direccionAval, lastEndorsmentId, 'AVAL', procTransaction)
        ...
        tableEndorsmentBD.columns.add('TELEFONO_MOVIL', VarChar, { nullable: true })
        tableEndorsmentBD.columns.add('CORREO_ELECTRONICO', VarChar, { nullable: true })
        ...
        const insertBulkData = await procTransaction.request().bulk(tableEndorsmentBD)
        ...
        return { message: 'Error durante la transacción', idEndorsment: 0, error: errorMessage };
    }
