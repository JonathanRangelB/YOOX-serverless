...
export const updateCustomer = async (
  ...
  try {
    ...
    const queryLimpiaAvalCliente =
      "DELETE FROM CLIENTES_AVALES WHERE ID_CLIENTE = @clientes_avales_id_cliente_delete ";
    poolRequest.input("clientes_avales_id_cliente_delete", sql.Int, id_cliente);

    let queryActualizaAvalCliente = "";

    if (id_aval) {
      queryActualizaAvalCliente =
        "INSERT INTO CLIENTES_AVALES (ID_CLIENTE, ID_AVAL) VALUES (@clientes_avales_id_cliente_insert, @clientes_avales_id_aval_insert) ";
      poolRequest.input(
        "clientes_avales_id_cliente_insert",
        sql.Int,
        id_cliente
      );
    ...
    const customerUpdate = await poolRequest.query(
